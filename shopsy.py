...
import os, time, random, json, uuid, asyncio, copy, traceback
...
async def run_sh_user_state(session_data):
    ...
    if st != 200:
        raise Exception(f"User state failed with status {st}: {resp_json}")
...
async def claim_gullak_tg(session_data):
    ...
    st, resp_json, headers, session_data = await asyncio.to_thread(sync_api_request, "POST", "/1/shopsy/games", body, session_data, True)
    if st != 200 or not resp_json.get("success"):
        raise Exception(f"Gullak claim failed: {resp_json}")
...
async def mine_account_parallel(session_data, progress_callback=None, parallel_count=4):
    ...
    phone = session_data.get("phone")
    if not phone:
        return {"status": "fail", "earned": 0, "msg": "Phone number missing in session."}
    ...
    try:
        ...
        if not initial_user_data:
            return {"status": "fail", "earned": 0, "msg": "Session expired or invalid. Please login again."}
        ...
        if not games:
            return {"status": "fail", "earned": 0, "msg": "No active games found for this account."}
        ...
        for i, g in enumerate(games):
            ...
            try:
                ...
                if progress_callback:
                    progress_callback(f"❌ Start game failed: {str(e)[:50]}")
                continue
            ...
            if game_sess_id:
                ...
                earned_this_game = success_count * 30
                ...
                if success_count > 0:
                    ...
                    try:
                        ...
                        if end_data:
                            ...
                            if progress_callback:
                                progress_callback(f"⚠️ Failed to end {game_name} (fallback also failed)")
                    except Exception as e:
                        if progress_callback:
                            progress_callback(f"❌ Fallback end error: {str(e)[:50]}")
            else:
                if progress_callback:
        ...
        traceback.print_exc()
        return {"status": "fail", "earned": 0, "msg": f"Unexpected error: {str(e)[:150]}"}
